...
fn run_u64_unif() {
    ...
    let dur = Null::<U64Tup, KVSpliceTx<U64Tup, U64Txn>>::new(0, 0, NULL_WRITE);
    // concurrency control, kv sparkle in this module
    let con = super::KVSplice::<U64Txn, U64Tup>::new();
    // service, multi-thread service
    let srv = MThreadService::new(8, KVSpliceTx::new, con, dur);
...
fn run_revm_10key() {
    ...
    for n in 1..=16 {
        ...
        let dur = Null::<EVMU256Tup, KVSpliceTx<EVMU256Tup, REVMInterpTxn>>::new(0, 0, NULL_WRITE);
        // concurrency control, kv sparkle in this module
        let con = super::KVSplice::<REVMInterpTxn, EVMU256Tup>::new();
        // service, multi-thread service
        let srv = MThreadService::new(n, KVSpliceTx::new, con, dur);
